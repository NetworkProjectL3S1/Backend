...
public class AuctionClientHandler {
    ...
    public String getUsername() {
        return authenticatedUser != null ? authenticatedUser.getUsername() : "Unauthenticated";
    }

    // Modified disconnect logic
    public void disconnect() {
        if (authenticatedUser != null) {
            userManager.removeActiveUser(authenticatedUser.getUsername());
        }
        try {
            server.clientDisconnected(this);
            channel.close();
        } catch (IOException e) {
            System.err.println("Error closing channel for " + getUsername() + ": " + e.getMessage());
        }
    }
}
